...
def main():
    ...
    # Selections mapping: (col, idx) -> tag
    fruit_colors = {}
    ...
    while running:
        ...
        # Precompute current rectangles for click hit-testing
        clickable_rects = []
        ...
        for e in pg.event.get():
            if e.type == pg.QUIT:
                ...
                if active_mode:
                    mx, my = e.pos
                    for rect, key, (x_cm, y_cm) in clickable_rects:
                        if rect.collidepoint(mx, my):
                            fruit_colors[key] = active_mode
                            # Build lists and save CSVs
                            reds = []
                            blacks = []
                            for (xc, yc, c, i) in positions:
                                tag = fruit_colors.get((c, i))
                                if tag == "red":
                                    reds.append((xc, yc))
                                elif tag == "black":
                                    blacks.append((xc, yc))
        ...
        hud_y += 20
        # Show first-row centers for quick verification in cm
